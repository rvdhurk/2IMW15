    ...
    with open(fileName) as file:
        ...
        for row in csvReader:
            ...
            if counter > limit and limit != -1:
                break
            tokens = tokenize(row[3], tokenizer=TweetTokenizer())
...
if __name__ == '__main__' :
    train = loadDataSet('sentiment.csv', 1000)
    l = int(len(train) * 0.8)
    test = train[l:]
    train = train[:l]
    wordId = 0
    ...
    TESTS = [
            ('5', WeightedAverageMutator(0.5), fitnessFunction, 100, len(words), -100, 100, 0.2, 0.5, 0.05),
            ('65',  WeightedAverageMutator(0.65), fitnessFunction, 100, len(words), -100, 100, 0.2, 0.5, 0.05),
            ('8',  WeightedAverageMutator(0.8), fitnessFunction, 100, len(words), -100, 100, 0.2, 0.5, 0.05),
            ('half',  HalfMutator(), fitnessFunction, 100, len(words), -100, 100, 0.2, 0.5, 0.05),
            ('sp',  SinglePointMutator(), fitnessFunction, 100, len(words), -100, 100, 0.2, 0.5, 0.05),
            ('ran',  RandomMutator(), fitnessFunction, 100, len(words), -100, 100, 0.2, 0.5, 0.05),
           ]
    for TEST in TESTS:
